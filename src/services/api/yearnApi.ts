...
export class YearnApiService {
  ...
  async getVaults(chainId: number = config.katanaChainId): Promise<YearnVault[]> {
    try {
  ...
  async getVaultByAddress(vaultAddress: string, chainId: number = config.katanaChainId): Promise<YearnVault | null> {
    try {
      const vaults: YearnVault[] = await this.getVaults(chainId);
      return (
        vaults.find((vault: YearnVault): boolean => vault.address.toLowerCase() === vaultAddress.toLowerCase()) || null
      );
    } catch (error) {
  ...
  getActiveStrategyAddresses(vault: YearnVault): string[] {
    return vault.strategies
      .filter((strategy): boolean =>
        Boolean(
          strategy.details?.totalDebt && strategy.details.totalDebt !== '0' && strategy.details.totalDebt !== '0x0'
        )
      )
