...
export class ContractReaderService {
  async getSteerPoolsFromStrategies(strategyAddresses: string[]): Promise<Record<string, string>> {
    ...
    try {
      ...
      const validSteerLps: { strategy: string; steerLp: string }[] = _.chain(steerLpResults)
        .map((result, index) =>
          result.status
            ? {
      ...
      const mappings: { strategy: string; pool: string }[] = _.chain(poolResults)
  ...
  async getMorphoVaultsFromStrategies(strategyAddresses: string[]): Promise<Record<string, string>> {
    try {
